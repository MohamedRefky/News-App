  ...
  HomeCubit(this.newsRepository) : super(const HomeInitial()) {
   ...
   void getTopHeadline({String? category}) async {
    try {
      ...
      final articles = await newsRepository.getTopHeadline(selectedCategory: state.selectedCategory);
emit(state.copyWith(
newsTopHeadlineList: articles,
topHeadlineStatus :RequestStatusEnum.loaded , errorMessage : null
));
